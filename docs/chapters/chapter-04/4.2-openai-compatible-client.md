# 4.2 OpenAI 兼容客户端设计

> 本节我们将设计 MiniClaw 的 LLM 客户端接口，为后续实现打下基础。

## 📋 本节目标

- 理解 OpenAI API 协议
- 设计统一的 LLM 客户端接口
- 定义请求和响应模型
- 为多模型适配做准备

## 🎯 本节要点

**OpenAI API 协议已成为行业标准**

几乎所有主流 LLM 都兼容 OpenAI API：
- Anthropic Claude（兼容模式）
- Google Gemini（兼容模式）
- 国产大模型（DeepSeek、智谱、文心等）
- 本地模型（Ollama、vLLM）

这意味着：**实现一个 OpenAI 兼容客户端 = 支持所有模型**

---

## 🏗️ 架构设计

### 分层架构

```
┌─────────────────────────────────┐
│  业务层（Controller/Service）    │
└─────────────────────────────────┘
              ↓
┌─────────────────────────────────┐
│  LlmClient 接口                  │  统一接口
└─────────────────────────────────┘
              ↓
┌─────────────────────────────────┐
│  OpenAiCompatibleLlmClient      │  实现类
│  - HTTP 请求构建                │
│  - 响应解析                     │
│  - 错误处理                     │
└─────────────────────────────────┘
              ↓
┌─────────────────────────────────┐
│  WebFlux WebClient              │  HTTP 客户端
└─────────────────────────────────┘
```

**设计原则：**

1. **接口抽象**：LlmClient 定义标准接口
2. **实现分离**：OpenAiCompatibleLlmClient 实现具体逻辑
3. **协议统一**：使用 OpenAI 兼容协议
4. **易于扩展**：未来可以添加其他协议支持

---

## 📦 核心模型

### 1. LlmClient 接口

```java
public interface LlmClient {

    /**
     * 同步调用 LLM
     * 等待完整结果后返回
     */
    LlmResponse chat(LlmRequest request);

    /**
     * 流式调用 LLM
     * 实时返回内容块
     */
    Flux<LlmChunk> stream(LlmRequest request);
}
```

**两种调用模式：**

| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **同步** | 等待完整响应 | 批处理、后台任务 |
| **流式** | 实时返回 chunks | 对话、实时展示 |

### 2. LlmRequest 请求模型

```java
@Data
@Builder
public class LlmRequest {
    private List<Message> messages;        // 消息列表
    private String model;                  // 模型名称
    private Double temperature;            // 温度参数
    private Integer maxTokens;             // 最大 token
    private Boolean stream;                // 是否流式
    private List<Map<String, Object>> tools;  // 工具定义
    private String toolChoice;             // 工具选择策略
}
```

**关键字段说明：**

| 字段 | 类型 | 说明 | 示例 |
|------|------|------|------|
| `messages` | List | 对话历史 | `[{"role": "user", "content": "你好"}]` |
| `model` | String | 模型名称 | `"gpt-4"`, `"claude-3-opus"` |
| `temperature` | Double | 随机性（0-2） | `0.7`（平衡）, `0`（确定性） |
| `maxTokens` | Integer | 最大输出 token | `4096` |
| `tools` | List | 工具定义（Function Calling） | 见下文 |

**Message 模型：**

```java
@Data
@Builder
public static class Message {
    private String role;                   // 角色
    private String content;                // 内容
    private List<ToolCall> toolCalls;      // 工具调用（assistant）
    private String toolCallId;             // 工具调用 ID（tool）
}
```

**角色说明：**

| 角色 | 说明 | 示例 |
|------|------|------|
| `system` | 系统指令 | "你是一个助手" |
| `user` | 用户输入 | "帮我写代码" |
| `assistant` | AI 响应 | "好的，我来帮你" |
| `tool` | 工具结果 | "执行结果：..." |

**便捷方法：**

```java
// 创建系统消息
Message.system("你是一个助手")

// 创建用户消息
Message.user("帮我写代码")

// 创建助手消息
Message.assistant("好的，我来帮你")

// 创建工具结果消息
Message.toolResult("call_123", "执行结果：...")
```

### 3. LlmResponse 响应模型

```java
@Data
@Builder
public class LlmResponse {
    private String content;                // 响应内容
    private List<ToolCall> toolCalls;      // 工具调用列表
    private String finishReason;           // 完成原因
    private Usage usage;                   // Token 统计
}
```

**finishReason 说明：**

| 值 | 说明 |
|----|------|
| `stop` | 正常结束 |
| `length` | 达到最大 token |
| `tool_calls` | 需要调用工具 |

**Usage 统计：**

```java
@Data
@Builder
public static class Usage {
    private Integer promptTokens;       // 输入 token
    private Integer completionTokens;   // 输出 token
    private Integer totalTokens;        // 总 token
}
```

### 4. LlmChunk 流式响应块

```java
@Data
@Builder
public class LlmChunk {
    private String delta;                   // 内容增量
    private List<ToolCall> toolCalls;       // 工具调用增量
    private String finishReason;            // 完成原因
    private boolean done;                   // 是否结束
}
```

**流式输出原理：**

```
LLM 生成："你好，我是 AI 助手"

同步响应：
[完整的 "你好，我是 AI 助手"]

流式响应（多个 chunks）：
[delta: "你"]
[delta: "好"]
[delta: "，"]
[delta: "我是"]
[delta: " AI"]
[delta: " 助手"]
[done: true]
```

**优势：**
- 用户感知更快（边生成边显示）
- 减少首字延迟
- 更好的用户体验

### 5. ToolCall 工具调用

```java
@Data
@Builder
public class ToolCall {
    private String id;                      // 调用 ID
    private String type;                    // 类型（function）
    private FunctionCall function;          // 函数调用
}
```

**Function Calling 流程：**

```
1. LLM 决定调用工具
   {
     "toolCalls": [{
       "id": "call_123",
       "function": {
         "name": "get_weather",
         "arguments": "{\"city\": \"北京\"}"
       }
     }]
   }

2. 应用执行工具
   get_weather("北京") → "晴天，25°C"

3. 返回工具结果
   Message.toolResult("call_123", "晴天，25°C")

4. LLM 基于结果继续生成
   "北京今天是晴天，气温25°C"
```

---

## 🔍 OpenAI API 协议

### 同步调用示例

**请求：**

```http
POST /v1/chat/completions HTTP/1.1
Host: api.openai.com
Authorization: Bearer sk-xxx
Content-Type: application/json

{
  "model": "gpt-4",
  "messages": [
    {"role": "user", "content": "你好"}
  ],
  "temperature": 0.7
}
```

**响应：**

```json
{
  "id": "chatcmpl-123",
  "object": "chat.completion",
  "created": 1700000000,
  "model": "gpt-4",
  "choices": [{
    "index": 0,
    "message": {
      "role": "assistant",
      "content": "你好！有什么我可以帮助你的吗？"
    },
    "finish_reason": "stop"
  }],
  "usage": {
    "prompt_tokens": 10,
    "completion_tokens": 15,
    "total_tokens": 25
  }
}
```

### 流式调用示例

**请求：**

```http
POST /v1/chat/completions HTTP/1.1
Host: api.openai.com
Authorization: Bearer sk-xxx
Content-Type: application/json

{
  "model": "gpt-4",
  "messages": [
    {"role": "user", "content": "你好"}
  ],
  "stream": true
}
```

**响应（SSE 格式）：**

```
data: {"id":"chatcmpl-123","choices":[{"delta":{"content":"你"},"index":0}]}

data: {"id":"chatcmpl-123","choices":[{"delta":{"content":"好"},"index":0}]}

data: {"id":"chatcmpl-123","choices":[{"delta":{"content":"！"},"index":0}]}

data: [DONE]
```

**SSE（Server-Sent Events）协议：**
- 每个 chunk 以 `data:` 开头
- 以空行分隔
- 最后发送 `data: [DONE]`

---

## 📊 模型兼容性

### 主流 LLM API 对比

| 模型 | API 兼容性 | 流式支持 | Function Calling |
|------|-----------|----------|-----------------|
| **OpenAI GPT** | 原生 | ✅ | ✅ |
| **Anthropic Claude** | 兼容模式 | ✅ | ✅ |
| **Google Gemini** | 兼容模式 | ✅ | ✅ |
| **DeepSeek** | 完全兼容 | ✅ | ✅ |
| **智谱 GLM** | 完全兼容 | ✅ | ✅ |
| **Ollama** | 完全兼容 | ✅ | ❌ |

**结论：** 实现 OpenAI 协议客户端 = 支持所有主流模型

---

## 🎓 设计思考

### 为什么用接口而不是直接实现？

**灵活性：**

```java
// 可以轻松切换实现
LlmClient client = new OpenAiCompatibleLlmClient("gpt-4");

// 未来可以添加其他实现
LlmClient client = new AnthropicClient("claude-3");
LlmClient client = new GeminiClient("gemini-pro");
```

**测试性：**

```java
// 可以轻松 mock
@MockBean
LlmClient llmClient;

when(llmClient.chat(any()))
    .thenReturn(LlmResponse.builder()
        .content("测试响应")
        .build());
```

### 为什么支持同步和流式两种模式？

**使用场景不同：**

```java
// 同步：适合批处理
List<String> results = prompts.stream()
    .map(prompt -> llmClient.chat(buildRequest(prompt)).getContent())
    .toList();

// 流式：适合实时对话
@GetMapping("/chat/stream")
public Flux<LlmChunk> streamChat(@RequestParam String prompt) {
    return llmClient.stream(buildRequest(prompt));
}
```

---

## 🔧 实际应用示例

### 场景 1：简单对话

```java
// 构建请求
LlmRequest request = LlmRequest.builder()
    .messages(List.of(
        Message.user("你好，介绍一下你自己")
    ))
    .build();

// 同步调用
LlmResponse response = llmClient.chat(request);
System.out.println(response.getContent());
```

### 场景 2：多轮对话

```java
List<Message> messages = new ArrayList<>();

// 第一轮
messages.add(Message.user("什么是 Java？"));
LlmResponse response1 = llmClient.chat(LlmRequest.builder()
    .messages(messages)
    .build());
messages.add(Message.assistant(response1.getContent()));

// 第二轮
messages.add(Message.user("它有什么特点？"));
LlmResponse response2 = llmClient.chat(LlmRequest.builder()
    .messages(messages)
    .build());
```

### 场景 3：流式输出

```java
Flux<LlmChunk> chunks = llmClient.stream(LlmRequest.builder()
    .messages(List.of(Message.user("写一首诗")))
    .build());

chunks.subscribe(chunk -> {
    if (chunk.getDelta() != null) {
        System.out.print(chunk.getDelta());  // 实时打印
    }
    if (chunk.isDone()) {
        System.out.println("\n[完成]");
    }
});
```

### 场景 4：Function Calling

```java
// 定义工具
Map<String, Object> weatherTool = Map.of(
    "type", "function",
    "function", Map.of(
        "name", "get_weather",
        "description", "获取城市天气",
        "parameters", Map.of(
            "type", "object",
            "properties", Map.of(
                "city", Map.of("type", "string", "description", "城市名称")
            )
        )
    )
);

// 调用 LLM
LlmResponse response = llmClient.chat(LlmRequest.builder()
    .messages(List.of(Message.user("北京今天天气怎么样？")))
    .tools(List.of(weatherTool))
    .build());

// 处理工具调用
if (response.hasToolCalls()) {
    ToolCall toolCall = response.getToolCalls().get(0);
    String city = parseArguments(toolCall.getFunction().getArguments());
    String result = getWeather(city);

    // 返回工具结果
    Message.toolResult(toolCall.getId(), result);
}
```

---

## ✅ 验证接口设计

### 编译检查

```bash
cd miniclaw-learn
mvn clean compile
```

**预期输出：**

```
[INFO] BUILD SUCCESS
```

### 结构检查

```bash
tree src/main/java/com/miniclaw/llm/
```

**预期结构：**

```
llm/
├── LlmClient.java
├── exception/
│   └── LlmException.java
└── model/
    ├── LlmRequest.java
    ├── LlmResponse.java
    ├── LlmChunk.java
    └── ToolCall.java
```

---

## 🐛 常见问题

### 1. 为什么要区分 Message.role？

不同角色有不同用途：
- `system`：设置 AI 行为
- `user`：用户输入
- `assistant`：AI 响应
- `tool`：工具返回

### 2. LlmChunk 和 LlmResponse 的区别？

- `LlmResponse`：完整响应（同步）
- `LlmChunk`：增量响应块（流式）

### 3. ToolCall 什么时候使用？

当 LLM 决定调用工具时（Function Calling），第 7 章会详细讲解。

---

## 📚 扩展阅读

- [OpenAI API 文档](https://platform.openai.com/docs/api-reference/chat)
- [OpenAI Function Calling](https://platform.openai.com/docs/guides/function-calling)
- [Server-Sent Events 协议](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events)

---

## 🎯 下一节

下一节我们将：
- 实现 OpenAiCompatibleLlmClient
- 处理 HTTP 请求和响应
- 解析 JSON 数据

[下一节：4.3 同步调用实现 →](./4.3-sync-implementation.md)
